// All the class names are modified according to the standardised mode, Be aware while updating the Version.
...
(function(root, factory) {
	...
	$.extend(Selectize.prototype, {
		...
		setup: function() {
			...
			var $dropdown_container;//Modified
			...
			$control_input    = $('<input type="text" autocomplete="off" />').attr("spellcheck","false").attr("maxlength","35").appendTo($control).attr('tabindex', tab_index);

			$dropdown_parent  = $(settings.dropdownParent || $wrapper);

			$dropdown         = $('<div>').addClass(settings.dropdownClass).addClass(classes).addClass(inputMode).hide().appendTo($dropdown_parent);

			$dropdown_container = $('<div>').addClass('dropdown_wrapper').appendTo($dropdown);//Modified

			$dropdown_content = $('<div>').addClass(settings.dropdownContentClass).appendTo($dropdown_container);//Modified
			...
			$dropdown.on('tap', '[data-selectable]', function() { return self.onOptionSelect.apply(self, arguments); });//LINE MODIFIED
